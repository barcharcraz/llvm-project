// RUN: %clang_cl_asan /std:c++17 /EHsc -Od %s -Fe%t
// RUN: not %run %t AsanReportTest 2>&1 | FileCheck %s --check-prefix=CHECK1
// RUN: %run %t UserHandlerTest 2>&1 | FileCheck %s --check-prefix=CHECK2
// RUN: %run %t ManyHandlersTest 2>&1 | FileCheck %s --check-prefix=CHECK3
// RUN: %run %t MultipleExceptionTest 2>&1 | FileCheck %s --check-prefix=CHECK4
// RUN: not %run %t NestedShadowExceptionTest 2>&1 | FileCheck %s --check-prefix=CHECK5
// RUN: %run %t RemovalTest 2>&1 | FileCheck %s --check-prefix=CHECK6
// RUN: %run %t RemoveOneTest 2>&1 | FileCheck %s --check-prefix=CHECK7
// RUN: %run %t RemoveAfterFirstExceptionTest 2>&1 | FileCheck %s --check-prefix=CHECK8
