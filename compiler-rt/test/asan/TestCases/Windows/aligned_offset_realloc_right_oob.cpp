...
int main(int argc, char **argv) {
  ...
  int i = atoi(argv[1]);
  buf = (char *)_aligned_offset_realloc(buf, 1024, alignments[i], 25);
  buf[1023] = 42;
  buf[1024] = 42;
  // CHECK: AddressSanitizer: heap-buffer-overflow on address [[ADDR:0x[0-9a-f]+]]
  // CHECK: WRITE of size 1 at [[ADDR]] thread T0
  // CHECK:   {{#0 .* main .*aligned_offset_realloc_right_oob.cpp}}:[[@LINE-3]]
  // CHECK: [[ADDR]] is located 7 bytes after 1024-byte region
  // CHECK: allocated by thread T0 here:
  // CHECK:   {{#0 .* _aligned_offset_realloc}}
  // CHECK:   {{#[1-2] .* main .*aligned_offset_realloc_right_oob.cpp}}:[[@LINE-9]]
  return 0;
}