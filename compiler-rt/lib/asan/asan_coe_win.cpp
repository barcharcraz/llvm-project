...
class Symbol {
 public:
  Symbol(HANDLE process, DWORD64 address)
    ...
    // We need to unlock then relock the registry to call into dbghlp.dll
    // because it can spawn a thread to accomplish symbolization, which can
    // deadlock if the registry is locked for CreateThread
